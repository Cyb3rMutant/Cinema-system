...
class Model:
    def __init__(self):
        ...
        for f in films:
            self.__films[f["FILM_TITLE"]] = Film(
                f["FILM_TITLE"],
                f["FILM_RATING"],
                f["FILM_GENRE"],
                f["FILM_YEAR"],
        ...
        for c in cities:
            c = City(
                c["CITY_NAME"],
                c["CITY_MORNING_PRICE"],
                c["CITY_AFTERNOON_PRICE"],
                c["CITY_EVENING_PRICE"],
            )
            ...
            for ci in cinemas:
                screens = dict()
                s_data = conn.select(
                    "SELECT * FROM screens WHERE CINEMA_ID=%s ORDER BY SCREEN_NUMBER;",
                    ci["CINEMA_ID"],
                )
                for s in s_data:
                    screens[s["SCREEN_ID"]] = Screen(
                        s["SCREEN_ID"],
                        s["SCREEN_NUM_VIP_SEATS"],
                        s["SCREEN_NUM_UPPER_SEATS"],
                        s["SCREEN_NUM_LOWER_SEATS"],
                        s["SCREEN_NUMBER"],
                    )
    ...
    def get_all_bookings_as_list(self):
        bookings = []
        data = conn.select(
            "SELECT b.BOOKING_REFERENCE, b.BOOKING_SEAT_COUNT, b.BOOKING_DATE, b.BOOKING_PRICE, b.SHOW_ID, b.SEAT_TYPE FROM bookings b\
                        ...
                        WHERE c.`CINEMA_ID`=%s AND ISNULL(b.`REFUND`);",
            self.__cinema.get_cinema_id(),
        )
        for b in data:
    ...
    def get_shows_for_listing(self, listing_id):
        ...
        data = conn.select(
            "SELECT `SHOW_ID`, `SHOW_TIME`, `SCREEN_ID` FROM shows WHERE LISTING_ID = %s;",
            listing_id,
        )
        try:
            sh_list = []
    ...
    def add_film(
        ...
        try:
            ...
            conn.insert(
                "INSERT INTO films VALUES(%s, %s, %s, %s, %s, %s, %s, %s);",
                film_title,
                film_rating,
                film_genre,
                film_year,
    ...
    def add_cinema(self, address, number_of_screens):
        try:
            ...
            conn.insert(
                "INSERT INTO cinemas (CINEMA_ADDRESS, CITY_NAME) VALUES (%s, %s);",
                address,
                self.__city.get_city_name(),
            )
            ...
            for screen in range(number_of_screens):
                conn.insert(
                    "INSERT INTO screens(SCREEN_NUM_VIP_SEATS,SCREEN_NUM_UPPER_SEATS,SCREEN_NUM_LOWER_SEATS,CINEMA_ID, SCREEN_NUMBER) VALUES (%s, %s, %s, %s, %s);",
                    10,
                    74,
                    36,
                    cinema_id,
    ...
    def validate_booking(self, seat_type, num_of_tickets):
        ...
        self.__booking_info = self.__show.add_booking(
            booking_reference,
            seat_type,
            num_of_tickets,
            datetime.date.today(),
    ...
    def add_booking(self, customer_email, name_on_card, card_number, cvv, expiry_date):
        ...
        customer.set_payment(Payment(name_on_card, card_number, expiry_date, cvv))
        self.__booking_info.set_customer(customer)

        conn.insert(
            "INSERT INTO bookings(BOOKING_REFERENCE, BOOKING_SEAT_COUNT, BOOKING_DATE, BOOKING_PRICE, SHOW_ID, SEAT_TYPE, CUSTOMER_EMAIL, USER_ID) VALUES (%s, %s, %s, %s, %s, %s, %s, %s);",
            self.__booking_info.get_booking_reference(),
            self.__booking_info.get_number_of_seats(),
            self.__booking_info.get_date_of_booking(),
            self.__booking_info.get_price(),
            self.__show.get_show_id(),
            self.__booking_info.get_seat_type(),
            self.__booking_info.get_customer().get_email(),
            self.__user.get_id(),
        )
    ...
    def update_customer(
        self, customer_name, customer_email, customer_phone, card_number
    ):
        self.__customers.update_customer(customer_email, customer_name, customer_phone)
        conn.update(
            "UPDATE customers SET CUSTOMER_NAME=%s, CUSTOMER_PHONE=%s, CARD_ENDING_DIGITS=%s",
            customer_name,
            customer_phone,
            card_number[-4:],
        )
    ...
    def add_customer(self, customer_name, customer_email, customer_phone, card_number):
        self.__customers.add_customer(customer_email, customer_name, customer_phone)
    ...
    def remove_listing(self):
        try:
            ...
            conn.delete("DELETE FROM listings WHERE LISTING_ID=%s;", id)
    ...
    def remove_show(self):
        try:
            for i in conn.select(
                "SELECT `CARD_ENDING_DIGITS`, `BOOKING_PRICE` FROM bookings b LEFT JOIN customers c ON b.`CUSTOMER_EMAIL`=c.`CUSTOMER_EMAIL` WHERE `SHOW_ID`=%s",
                self.__show.get_show_id(),
            ):
    ...
    def add_listing(self, film):
        conn.insert(
            "INSERT INTO listings (LISTING_DATE, FILM_TITLE, CINEMA_ID) VALUES (%s, %s, %s);",
            self.__date,
            film,
            self.__cinema.get_cinema_id(),
        )
        # now get the listing id from database
        listing_id = conn.select("SELECT MAX(LISTING_ID) as LISTING_ID FROM listings")[
            0
        ]["LISTING_ID"]
    ...
    def update_listing(self, film):
        ...
        conn.update(
            "UPDATE listings SET LISTING_DATE=%s, FILM_TITLE=%s WHERE LISTING_ID=%s;",
            self.__date,
            film.get_title(),
            listing_id,
        )
    ...
    def staff_number_of_bookings(self, start, end):
        return conn.select(
            "SELECT u.`USER_ID`, u.`USER_NAME`, COUNT(b.`BOOKING_REFERENCE`) as tot\
                    FROM users u\
    ...
    def get_city_price(self):
        try:  # Throws an error if no show is selected, returning -1 prevents
            time = conn.select(
                "SELECT NAME FROM times_of_day WHERE %s BETWEEN START_TIME AND END_TIME;",
                self.__show.get_time(),
            )[0]["NAME"]
        except:
            return
        if time == "morning":
    ...
    def get_cinema_city(self):
        city_name = conn.select(
            "SELECT CITY_NAME FROM cinemas WHERE CINEMA_ID = %s;",
            self.__cinema.get_cinema_id(),
        )[0]["CITY_NAME"]
    ...
    def cancel_booking(
        ...
        b.get_customer().set_payment(
            Payment(name_on_card, card_number, cvv, expiry_date)
        )
        conn.update(
            "UPDATE bookings SET REFUND=%s WHERE BOOKING_REFERENCE=%s;",
            self.__show.get_bookings()[booking_reference].get_price() / 2,
            booking_reference,
        )
    ...
    def add_show(self, time):
        try:
            ...
            data = conn.select(
                "SELECT `SCREEN_ID` FROM screens WHERE `SCREEN_ID` NOT IN ( (\
        ...
        ",
            ...
            conn.insert(
                "INSERT INTO shows(SHOW_TIME, SCREEN_ID, LISTING_ID) VALUES (%s, %s, %s);",
                time,
                screen_id,
                self.__listing.get_listing_id(),
            )

            show_id = conn.select("SELECT MAX(SHOW_ID) as SHOW_ID FROM shows")[0][
                "SHOW_ID"
            ]
    ...
    def add_user(self, name, password, user_type):
        ...
        conn.insert(
            "INSERT INTO users(USER_NAME, USER_PASSWORD_HASH, USER_TYPE, CINEMA_ID) VALUES (%s, %s, %s, %s);",
            name,
            password,
            user_type,
            self.__cinema.get_cinema_id(),
        )
        data = conn.select("SELECT MAX(user_id) as USER_ID FROM users;")
    ...
    def get_booking(self, booking_ref):
        try:
            if isinstance(self.__user, Admin):
                data = conn.select(
                    "SELECT b.`BOOKING_REFERENCE`, b.`SHOW_ID`, s.`LISTING_ID`, l.`CINEMA_ID`, c.`CITY_NAME`\
                                    FROM bookings b\
                                        LEFT JOIN shows s ON b.`SHOW_ID` = s.`SHOW_ID`\
                                        LEFT JOIN listings l ON s.`LISTING_ID` = l.`LISTING_ID`\
                                        LEFT JOIN cinemas c ON l.`CINEMA_ID` = c.`CINEMA_ID`\
                ...
                data = conn.select(
                    "SELECT b.`BOOKING_REFERENCE`, b.`SHOW_ID`, s.`LISTING_ID`, l.`CINEMA_ID`, c.`CITY_NAME`\
                                    FROM bookings b\
                                        ...
                                        LEFT JOIN cities ON c.`CITY_NAME` = cities.`CITY_NAME`\
                                    WHERE\
                                        b.`BOOKING_REFERENCE` = %s AND c.`CINEMA_ID` = %s;",
                    booking_ref,
    ...
    def get_user_types(self):
        return [
            t["USER_TYPE"] for t in conn.select("SELECT USER_TYPE FROM user_types;")
        ]
    ...
    def set_listing(self, listing):
        ...
        for s in shows:
            ...
            self.__listing.add_show(
                s["SHOW_ID"],
                s["SHOW_TIME"],
                self.__cinema.get_screens()[s["SCREEN_ID"]],
            )
    ...
    def set_show(self, show):
        ...
        bookings = conn.select(
            "SELECT * FROM bookings WHERE SHOW_ID=%s AND ISNULL(REFUND)",
            self.__show.get_show_id(),
        )
        print(bookings)
        for b in bookings:
            self.__show.add_booking(
                b["BOOKING_REFERENCE"],
                b["SEAT_TYPE"],
                b["BOOKING_SEAT_COUNT"],
                b["BOOKING_DATE"],
                b["BOOKING_PRICE"],
                self.__customers[b["CUSTOMER_EMAIL"]],
            )
