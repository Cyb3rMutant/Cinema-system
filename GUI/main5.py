...
class Login:
    def __init__(self, window):
        ...
        self.login_label = tk.Label(
            self.frame, text='Login', borderwidth=1, bg='#333333', fg='#DD2424', font=("Arial", 32))
        self.username_label = tk.Label(
            self.frame, text='Username', bg='#333333', fg='#FFFFFF', font=("Arial", 18))
        self.username_entry = tk.Entry(self.frame, font=("Arial", 18))
        self.password_label = tk.Label(
            self.frame, text='Password', bg='#333333', fg='#FFFFFF', font=("Arial", 18))
        self.password_entry = tk.Entry(self.frame, show='*', font=("Arial", 18))
        self.login_button = tk.Button(self.frame, text='Login', bg='#DD2424', fg='#000000', font=(
            "Arial", 18), command=self.login_verify)
    ...
    def login_verify(self):
        ...
        if dbcursor.rowcount < 1:
            ...
            if sha256_crypt.verify(self.password, str(data[1])):
                tk.messagebox.showinfo(title="Login Successful",
...
class BaseWindow():
    def __init__(self, window):
        ...
        self.MainFrame = tk.Frame(self.window, bd=10, width=1350,
                                  height=800, bg='gainsboro', relief=tk.RIDGE)
        self.MainFrame.grid()
        ...
        self.BodyFrame = tk.Frame(
            self.MainFrame, bd=10, width=1330, height=670, bg="gainsboro", relief=tk.RIDGE)
        self.BodyFrame.grid()
        ...
        self.branch_label = tk.Label(self.HeaderFrame, text="Bristol City Centre",
                                     borderwidth=1, bg='#333333', fg='#DD2424', font=("Arial", 16))
        # Higher width overlaps border. Had to make it smaller and adjust with X
        self.branch_label.place(x=890, y=5, width=400)
        self.name_label = tk.Label(
            self.HeaderFrame, text="James Jenkins [Booking Staff]", borderwidth=1, bg='#333333', fg='#DD2424', font=("Arial", 16))
...
class Dashboard(BaseWindow):
    def __init__(self, window):
        ...
        # Page Title
        self.page_label = tk.Label(self.HeaderFrame, text="Dashboard",
                                   borderwidth=1, bg='#333333', fg='#DD2424', font=("Arial", 16))
        self.page_label.place(x=0, y=5, width=443, height=70)  # 443 = width/3

        # Body
        self.create_booking_btn = tk.Button(
            self.BodyFrame, text="Create Booking", borderwidth=1, command=self.create_booking_gui)
        self.view_bookings_btn = tk.Button(
            self.BodyFrame, text="View Bookings", borderwidth=1)
        self.view_film_listings_btn = tk.Button(
            self.BodyFrame, text="View Film Listings", borderwidth=1)
        self.cancel_booking_btn = tk.Button(
            self.BodyFrame, text="Cancel Booking", borderwidth=1)

        self.add_listing_btn = tk.Button(
            self.BodyFrame, text="Add Listing", borderwidth=1)
        self.remove_listing_btn = tk.Button(
            self.BodyFrame, text="Remove Listing", borderwidth=1)
        self.update_listing_btn = tk.Button(
            self.BodyFrame, text="Update Listing", borderwidth=1)
        self.generate_report_btn = tk.Button(
            self.BodyFrame, text="Generate Report", borderwidth=1)

        self.add_new_cinema_btn = tk.Button(
            self.BodyFrame, text="Add New Cinema", borderwidth=1)
        self.add_listing_master_btn = tk.Button(
            self.BodyFrame, text="Add Listing to Cinema", borderwidth=1)
...
class BookingGUI(BaseWindow):
    def __init__(self, window):
        ...
        # Page Name
        self.page_label = tk.Label(self.HeaderFrame, text="Create Booking",
                                   borderwidth=1, bg='#333333', fg='#DD2424', font=("Arial", 16))
        self.page_label.place(x=0, y=5, width=443, height=70)  # 443 = width/3
